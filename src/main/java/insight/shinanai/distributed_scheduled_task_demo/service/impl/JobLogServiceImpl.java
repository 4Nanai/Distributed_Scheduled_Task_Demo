...
public class JobLogServiceImpl extends ServiceImpl<JobLogMapper, JobLog>
    ...
    @Override
    public void sendAndSaveLog(Long jobId,
                               String jobName,
                               String logLevel,
                               String message,
                               int shardItem,
                               String executionId) {
        JobLog jobLog = new JobLog(null, jobId, jobName, shardItem, logLevel, message, null, executionId);
        this.save(jobLog);
    ...
    @Override
    public List<LogVO> getRecentLogsLimitHistoryCount(String jobId, int executeLogHistoryCount) {
        List<JobLog> jobLogs = this.getBaseMapper()
                .getRecentLogsLimitHistoryCount(jobId, executeLogHistoryCount);
